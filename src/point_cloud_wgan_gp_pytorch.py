...
def train(train_data, epochs, batch_size, num_points, G, D, G_optimizer, D_optimizer, args, log_file, log_file2, epoch, discriminator_updates, gradient_penalty_coefficient):
    """
    Train for one epoch
    """
    ...
    for iteration in range(n_batches):
        ...
        for update in range(discriminator_updates):
...
def generate(number_samples, generator):
    """
    Generate point cloud samples for feedback while training
    """
...
def main(args):
    ...
    LOG_FILE_NAME = "plots/wgan_training_predictions.csv"
    LOG_FILE_NAME2 = "plots/wgan_training_loss.csv"
    ...
    if args.should_continue:
        generator_checkpoint = torch.load('checkpoints/generator/model_best.pth')
        discriminator_checkpoint = torch.load('checkpoints/discriminator/model_best.pth')
    ...
    for epoch in range(first_epoch, args.epochs):
        ...
        loss_g, loss_d, G_updated, D_updated = train(all_pc_data, args.epochs, args.train_batch, args.num_points, G, D, G_optimizer, D_optimizer, args, log_file1, log_file2, epoch, discriminator_updates, gradient_penalty_coefficient)
        G = G_updated
        D = D_updated
...
if __name__ == "__main__":
    ...
    parser.add_argument("--discriminator_updates", default=5, type=int, help="Discriminator updates per generator update")
    ...
    parser.add_argument("--train_batch", default=50, type=int, help="Batch size for training")
